...
"""
        ...
        3. 统计信息文件
        4. 多余碱基统计摘要文件（可选）
            包含一个推荐去除的序列IDS列表文件：recommended_removal.txt
                (1) 两端都具有端粒的序列不进行额外处理，即使它是其序列中的一小部分；
                (2) 只有单端端粒的序列，我们对其进行多余碱基的计算，并求出均值和Q3值；
                (3) 若全部的多余碱基均值大于Q3，则挑选出多余碱基大于均值的序列；
                (4) 并将其ids,写入文件中，每行一个，是否去除自行决定。
        5. 多余碱基统计文件（可选）
...
def write_statistics(non_telomere_sequences, telomere_sequences, one_end_telomere_sequences, 
    ...
    try:
        with open(output_stat_file, 'w') as stat_f:
            ...
            stat_f.write(f"非端粒序列\t{len(non_telomere_sequences)}\t{sum(map(len, non_telomere_sequences.values()))}\t{sum(map(len, non_telomere_sequences.values()))/1000000}\n")
            stat_f.write(f"端粒序列\t{len(telomere_sequences)}\t{sum(map(len, telomere_sequences.values()))}\t{sum(map(len, telomere_sequences.values()))/1000000}\n")
            stat_f.write(f"一端端粒序列\t{len(one_end_telomere_sequences)}\t{sum(map(len, one_end_telomere_sequences.values()))}\t{sum(map(len, one_end_telomere_sequences.values()))/1000000}\n")
            stat_f.write(f"两端端粒序列\t{len(both_ends_telomere_sequences)}\t{sum(map(len, both_ends_telomere_sequences.values()))}\t{sum(map(len, both_ends_telomere_sequences.values()))/1000000}\n")
            stat_f.write(f"一端端粒序列多余碱基\t{sum(map(lambda x: x[0], one_end_excess_counts.values()))}\t{sum(map(lambda x: x[1], one_end_excess_counts.values()))}\t{sum(map(lambda x: x[1], one_end_excess_counts.values()))/1000000}\n")
            stat_f.write(f"两端端粒序列多余碱基\t{sum(map(lambda x: x[0], one_end_excess_counts.values()))}\t{sum(map(lambda x: x[1], one_end_excess_counts.values()))}\t{sum(map(lambda x: x[1], one_end_excess_counts.values()))/1000000}\n")
    except Exception as e:
