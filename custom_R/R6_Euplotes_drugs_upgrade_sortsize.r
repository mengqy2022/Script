...
PlottingClass <- R6Class("PlottingClass",
  public = list(
    ...
    factor_reorder = function(data, var) {
      ...
      # 按数据输入的变量顺序排序
      data$Concentration <- factor(data$Concentration, levels = unique(data$Concentration))
      return(data)
    },
    ...
    boxplot_pvalue = function(step_increase, days_col = "Days", ref_group = "Control", fill_color = c("red", "green", "#9ED8DB", "#3498DB")) {
      ...
      # df_p_val <- self$data %>%
      #   wilcox_test(Totals ~ Concentration, ref.group = ref_group, paired = TRUE) %>%
      ...
      Record_1$Concentration <- reorder(Record_1$Concentration, Record_1$Totals, FUN = mean)
      Record_1$Concentration <- factor(Record_1$Concentration, levels = rev(levels(Record_1$Concentration)))
...
create_plots <- function(data, file_name_prefix, titles, 
  ...
  if (plot_class == "smooth_box") {
    ...
    ggsave(paste0(file_name_prefix, "_smooth_box.pdf"), combined_plot, width = 35, height = 20, units = "cm") #+ 15
