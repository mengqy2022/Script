...
#         geom_smooth(aes(linetype = !!sym(concentration_col)), se = F, alpha = .05) +
...
PlottingClass <- R6Class("PlottingClass",
												 public = list(
												 	...
												 	plot_smooth = function(days_col = "Days", concentration_col = "Concentration", 
												 		...
												 		# 计算均值、标准差（SD）和标准误差（SE）
												 		Record_1 <- self$data %>%
												 			group_by(!!sym(concentration_col), !!sym(days_col)) %>%
												 			summarise(
												 				mean_totals = as.integer(mean(!!sym(totals_col), na.rm = TRUE)),
												 				sd = sd(!!sym(totals_col), na.rm = TRUE),  # 计算标准差
												 				n = n(),                                   # 计算每组样本数
												 				se = sd / sqrt(n),                         # 计算标准误差
												 				.groups = 'drop'
												 			)
												 		...
												 		n_total <- length(unique(self$data$Repeat)) * length(unique(self$data$Times))
												 		
												 		max_days <- max(Record_1[[days_col]], na.rm = TRUE)
												 		
												 		plot <- ggplot(Record_1, aes(x = !!sym(days_col), y = mean_totals, color = !!sym(concentration_col))) +
												 			geom_smooth(aes(linetype = !!sym(concentration_col)), se = FALSE, alpha = 0.05) +
												 			geom_point(size = 3, alpha = 0.8, 
												 								 #position = position_dodge(width = 0.2)
												 								 ) +
												 			# 添加误差棒（使用标准误差SE），与点使用相同的dodge宽度
												 			geom_errorbar(
												 				aes(ymin = mean_totals - se, ymax = mean_totals + se),
												 				width = 0.1,
												 				#position = position_dodge(width = 0.2)  # 与点的dodge宽度保持一致
												 			) +
												 			self$set_theme() +
												 			scale_x_continuous(breaks = seq(0, max_days, by = 1)) +
												 			xlab("Days") +
												 			ylab("Cell numbers") +
												 			annotate("text", x = max_days, y = -15, label = paste("n =", n_total), colour = "black", fontface = 'bold') +
												 			scale_color_manual(values = fill_color) +
												 			guides(color = guide_legend(override.aes = list(size = 6), ncol = 6)) +
												 			theme(
